...
mydata <- read.table("D:/mvc_bimonth_raw_3067/from_8km/jja.txt",sep=" ",fill=FALSE, strip.white=TRUE,header = TRUE)
...
vip_v<-matrix(, nrow = 3911, ncol = 13)
vip_loc<-matrix(, nrow = 3911, ncol = 13)
coefficients<-matrix(, nrow = 3911, ncol = 13)
...
for (i in 1:3911){
  ...
  for (j in 1:34){
    ...
    v1<-x_pre[3,] # 3 represents climate values in JJA 
    ...
    v7<-x_pre[2,] # 2 represents climate values in MAM 
    v8<-x_spei[2,]# for spei,include -Inf values
    v9<-x_tmax[2,]
    v10<-x_tmin[2,]
    v11<-x_tmean[2,]
    v12<-x_rad[2,]
    ...
    X<-cbind(v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12)
    vecy<-as.vector(Y)
    if (any(is.na(X))|any(is.infinite(X))|any(is.na(Y))|all(vecy == vecy[1])){
      plsres[i,2:4]=NA
      vip_v[i,2:13]=NA
      vip_loc[i,2:13]=NA
      coefficients[i,2:13]=NA
    }else{
    ncomp = 1 
...
write.table(plsres, file="D:/mvc_bimonth_raw_3067/plsres_new1_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_v, file="D:/mvc_bimonth_raw_3067/vip_v_new1_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_loc, file="D:/mvc_bimonth_raw_3067/vip_loc_new1_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(coefficients, file="D:/mvc_bimonth_raw_3067/coefficients_new1_ncomp1.txt", row.names=FALSE, col.names=TRUE)
