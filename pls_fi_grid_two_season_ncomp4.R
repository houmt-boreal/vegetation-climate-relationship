...
mydata <- read.table("D:/mvc_bimonth_raw_3067/from_10km/mean/son_mean.txt",sep=" ",fill=FALSE, strip.white=TRUE,header = TRUE)
...
for (i in 1:3911){
  ...
  for (j in 1:34){
    ...
    v1<-x_pre[4,]  # 2 represents climate values in MAM 
    v2<-x_spei[4,]# for spei,include -Inf values
    v3<-x_tmax[4,]
    v4<-x_tmin[4,]
    v5<-x_tmean[4,]
    v6<-x_rad[4,]
    
    v7<-x_pre[3,] # 2 represents climate values in MAM 
    v8<-x_spei[3,]# for spei,include -Inf values
    v9<-x_tmax[3,]
    v10<-x_tmin[3,]
    v11<-x_tmean[3,]
    v12<-x_rad[3,]
    ...
    # get vip
    if (Q2_1>0.0975 & Q2_2>0.0975 & Q2_3>0.0975 & Q2_4>0.0975) # means having 4th component
    {ttt<-as.vector(pls_res.vip[,4])# for including 4th component
    s<-length(ttt)
    for (ss in 1:s){
      jj<-sort(ttt, TRUE)[ss]
...
write.table(plsres, file="D:/plsres_son_2season_comp4.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_v, file="D:/vip_v_son_2season_comp4.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_loc, file="D:/vip_loc_son_2season_comp4.txt", row.names=FALSE, col.names=TRUE)
