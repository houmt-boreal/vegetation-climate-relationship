...
coefficients<-matrix(, nrow = 3911, ncol = 25)
...
for (i in 1:3911){
  ...
  for (j in 1:34){
    ...
    for (ss in 1:s){
      ...
      coefficients[i,2:25]<-NA
      vip_v[i,2:25]<-NA
      vip_loc[i,2:25]<-NA}
    }
}
write.table(plsres, file="D:/plsres_season_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_v, file="D:/vip_v_season_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(vip_loc, file="D:/vip_loc_season_ncomp1.txt", row.names=FALSE, col.names=TRUE)
write.table(coefficients, file="D:/coefficients_season_ncomp1.txt", row.names=FALSE, col.names=TRUE)
